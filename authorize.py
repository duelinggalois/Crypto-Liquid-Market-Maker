...
class GdaxAuth(AuthBase):
    ...
    def __call__(self, request):
        ...
        message = timestamp + request.method + request.path_url + \
            (request.body or '')
        message = message.encode() #hmac does not accpet strings
...
def run_Subscribe(pair, channel):
    ...
    return Subscribe(pair, channel=channel, auth=True, api_key=keys['api_key'], 
                api_secret=keys['api_secret'], api_passphrase=keys['api_pass'])
