...
class Order(BaseWrapper):
  ...
  def __repr__(self):
    return (
      "<Order pair:{}, side:{}, size:{}, price:{}, filled:{}, status:{}, "
      "post_only:{}, exchange_id:{}>").format(
      self.pair, self.side, self.size, self.price, self.filled, self.status,
      self.post_only, self.exchange_id
    )
