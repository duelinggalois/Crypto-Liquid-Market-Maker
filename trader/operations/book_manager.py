...
class AbstractBookManager(abc.ABC):
  ...
  def send_trade_sequence(self, filled_order):
    """
    Sends trades in a operations in response to a filled trade. The first size
...
class BookManager(AbstractBookManager):
  ...
  def send_orders(self, adjust_queue=None):
    ...
    pick_sell = True
    max_size = self.terms.min_size + (
      self.terms.count - 1) * self.terms.size_change
    while len(buys) + len(sells) > 0:
      if adjust_queue is not None and adjust_queue.qsize() > 0:
        # Another thread is taking priority stopping or pausing
        while adjust_queue.qsize() > 0:
  ...
  def send_order(self, order):
    ...
    if order.status not in ["open", "filled"]:
